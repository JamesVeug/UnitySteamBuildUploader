...
namespace Wireframe
{
    ...
    public partial class PlayStationUploadDestination : AUploadDestination
    {
        ...
        public override void TryGetErrors(List<string> errors)
        {
            ...
            if (!InternalUtils.GetService<PlayStationService>().IsReadyToStartBuild(out string serviceReason))
            {
            ...
            else
            {
                ...
                {
                    errors.Add($"PlayStation Title '{m_app.Name}' does not have a Title ID set.");
                }

                if (string.IsNullOrEmpty(m_app.ContentID))
                {
                    errors.Add($"PlayStation Title '{m_app.Name}' does not have a Content ID set.");
                }
            }
