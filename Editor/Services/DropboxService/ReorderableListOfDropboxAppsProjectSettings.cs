...
namespace Wireframe
{
    public class ReorderableListOfDropboxAppsProjectSettings : InternalReorderableList<DropboxConfig.DropboxApp>
    {
        protected override void DrawItem(Rect rect, int index, bool isActive, bool isFocused)
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                GUI.Label(rect1, new GUIContent("Name", "Display name for this Dropbox app. UI only — not sent to Dropbox."));
                rect1.x += rect1.width;
                rect1.width = width;
